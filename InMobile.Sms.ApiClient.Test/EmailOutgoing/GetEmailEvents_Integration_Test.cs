...
public class GetEmailEvents_Integration_Test
{
    ...
    [Fact]
    public async Task GetEmailEventsAsync_Success_Test()
    {
        var responseJson = @"{
    ""events"": [
        {
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var response = await client.EmailOutgoing.GetEmailEventsAsync(limit: 10);
            Assert.Single(response.Events);
            var singleEvent = response.Events.Single();
            Assert.NotNull(singleEvent);
            Assert.Equal(new OutgoingEmailId("3baba088-4029-49f7-b90d-1e44b56e36c6"), singleEvent.MessageId);
    ...
    [Fact]
    public async Task GetEmailEventsAsync_ApiError_Test()
    {
        var responseJson = @"{
    ""errorMessage"": ""Domain not validated"",
    ""details"": []
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var ex = await Assert.ThrowsAsync<InMobileApiException>(() => client.EmailOutgoing.GetEmailEventsAsync(limit: 10));
    ...
    [InlineData(251, false)]
    [InlineData(1000, false)]
    public async Task GetEmailEventsAsync_Limit_Test(int limit, bool expectApiCalled)
    {
        var emptyResponseJson = @"{
    ""events"": [
]}";
        ...
        using (var server = UnitTestHttpServer.StartOnAnyAvailablePort(new RequestResponsePair(request: expectedRequest, response: responseToSendback)))
        {
            ...
            if (expectApiCalled)
            {
                var response = await client.EmailOutgoing.GetEmailEventsAsync(limit: limit);
                Assert.Empty(response.Events);
            }
            else
            {
                _ = await Assert.ThrowsAsync<ArgumentException>(() => client.EmailOutgoing.GetEmailEventsAsync(limit: limit));
            }
        }
    }
}