...
public class ClientException_Test
{
    ...
    [Fact]
    public async Task EnsureNetworkExceptionsAreVisible_Async_Test()
    {
        var client = new InMobileApiClient(new InMobileApiKey("Some_Key"), baseUrl: $"http://localhost:{50000}");
        _ = await Assert.ThrowsAsync<WebException>(() => client.SmsOutgoing.GetStatusReportsAsync(limit: 10));
    }
...
""Go away""
    ...
    [InlineData("409 Conflict", 409)]
    [InlineData("500 InternalServerError", 500)]
    public async Task InMobileApiErrorReturned_Async_Test(string statusCodeString, int expectedStatusCode)
    {
        var responseJson = @"{
""errorMessage"": ""Forbidden thing"",
""details"": [
...
""Go away""
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var exceptionThrown = await Assert.ThrowsAsync<InMobileApiException>(() => client.SmsOutgoing.GetStatusReportsAsync(limit: 12));
            Assert.Equal(expectedStatusCode, (int)exceptionThrown.ErrorHttpStatusCode);
            Assert.Equal($"{statusCodeString}: Forbidden thing. You shall not pass; Go away", exceptionThrown.Message);
        }
    }
