...
public class GetById_Integration_Test
{
    ...
    public async Task GetByIdAsync_Success_Test() 
    {
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var entry = await client.SmsTemplates.GetByIdAsync(templateId: new SmsTemplateId("some_template_id"));
    ...
    public async Task GetByIdAsync_ApiError_NotFound_Test()
    {
        ...
        using (var server = UnitTestHttpServer.StartOnAnyAvailablePort(new RequestResponsePair(request: expectedRequest, response: responseToSendback)))
        {
            ...
            var ex = await Assert.ThrowsAsync<InMobileApiException>(() => client.SmsTemplates.GetByIdAsync(new SmsTemplateId("some_template_id")));
            Assert.Equal(HttpStatusCode.NotFound, ex.ErrorHttpStatusCode);
        }
    }
