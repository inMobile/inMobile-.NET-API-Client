...
public class UpdateList_Integration_Test
{
    ...
    [Fact]
    public async Task UpdateListAsync_WithRecipientList_Test()
    {
        var responseJson = @"{                
                ""name"": ""New name"",
                ""id"": ""some_list_id"",
        ...
        using (var server = UnitTestHttpServer.StartOnAnyAvailablePort(new RequestResponsePair(request: expectedRequest, response: responseToSendback)))
        {
            ...
            // Execute
            var entry = await client.Lists.UpdateListAsync(list: list);
            Assert.Equal("New name", entry.Name);
            Assert.Equal("some_list_id", entry.Id.Value);
            Assert.Equal(new DateTime(2001, 02, 24, 14, 50, 23, DateTimeKind.Utc), entry.Created);
        }
    ...
    [Fact]
    public async Task UpdateListAsync_WithRecipientListUpdateInfo_Test()
    {
        var responseJson = @"{                
                ""name"": ""New name"",
                ""id"": ""some_list_id"",
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var entry = await client.Lists.UpdateListAsync(list: new RecipientListUpdateInfo(listId: new RecipientListId("some_list_id"), name: "New name"));
            Assert.Equal("New name", entry.Name);
            Assert.Equal("some_list_id", entry.Id.Value);
            Assert.Equal(new DateTime(2001, 02, 24, 14, 50, 23, DateTimeKind.Utc), entry.Created);
        }
...
""Go away""
    ...
    [Fact]
    public async Task UpdateListAsync_ApiError_WithRecipientList_Test()
    {
        var responseJson = @"{
""errorMessage"": ""Forbidden thing"",
""details"": [
...
""Go away""
        ...
        using (var server = UnitTestHttpServer.StartOnAnyAvailablePort(new RequestResponsePair(request: expectedRequest, response: responseToSendback)))
        {
            ...
            // Execute
            var ex = await Assert.ThrowsAsync<InMobileApiException>(() => client.Lists.UpdateListAsync(list: list));
...
""Go away""
    ...
    [Fact]
    public async Task UpdateListAsync_ApiError_WithRecipientListUpdateInfo_Test()
    {
        var responseJson = @"{
""errorMessage"": ""Forbidden thing"",
""details"": [
...
""Go away""
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var ex = await Assert.ThrowsAsync<InMobileApiException>(() => client.Lists.UpdateListAsync(list: new RecipientListUpdateInfo(listId: new RecipientListId("some_list_id"), name: "New name")));
