...
public class GetListById_Integration_Test
{
    ...
    [Fact]
    public async Task GetListByIdAsync_Test()
    {
        var responseJson = @"{                
                ""name"": ""Some list name"",
                ""Id"": ""some_list_id"",
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var entry = await client.Lists.GetListByIdAsync(listId: new RecipientListId("some_list_id"));
            Assert.Equal("Some list name", entry.Name);
            Assert.Equal("some_list_id", entry.Id.Value);
            Assert.Equal(new DateTime(2001, 02, 24, 14, 50, 23, DateTimeKind.Utc), entry.Created);
        }
    ...
    [Fact]
    public async Task GetListByIdAsync_ApiError_NotFound_Test()
    {
        var responseJson = @"{
                 ""errorMessage"": ""Could not find list: some_list_id"",
                 ""details"": []
        ...
        using (var server = UnitTestHttpServer.StartOnAnyAvailablePort(new RequestResponsePair(request: expectedRequest, response: responseToSendback)))
        {
            ...
            var ex = await Assert.ThrowsAsync<InMobileApiException>(() => client.Lists.GetListByIdAsync(listId: new RecipientListId("some_list_id")));
            Assert.Equal(HttpStatusCode.NotFound, ex.ErrorHttpStatusCode);
        }
    }
    ...
    [Fact]
    public async Task GetListByIdAsync_ApiError_InternalServerError_Test()
    {
        var responseJson = @"{
                ""errorMessage"": ""Forbidden thing"",
                ""details"": [
