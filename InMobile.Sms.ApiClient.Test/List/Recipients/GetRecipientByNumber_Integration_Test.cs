...
public class GetRecipientByNumber_Integration_Test
{
    ...
    [Fact]
    public async Task GetRecipientByNumberAsync_Test()
    {
        var responseJson = @"{
                                    ""externalCreated"": ""2001-02-10T14:50:23Z"",
                                    ""numberInfo"": {
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var recipient = await client.Lists.GetRecipientByNumberAsync(listId: new RecipientListId("some_list_id"), numberInfo: new NumberInfo(countryCode: "45", phoneNumber: "1111"));
    ...
    [Fact]
    public async Task GetRecipientByNumberAsync_ApiError_NotFound_Test()
    {
        var responseJson = @"{
                ""errorMessage"": ""Could not find recipient: ..."",
                ""details"": []
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var ex = await Assert.ThrowsAsync<InMobileApiException>(() => client.Lists.GetRecipientByNumberAsync(listId: new RecipientListId("some_list_id"), numberInfo: new NumberInfo(countryCode: "45", phoneNumber: "1111")));
    ...
    [Fact]
    public async Task GetRecipientByNumberAsync_ApiError_InternalServerError_Test()
    {
        var responseJson = @"{
                ""errorMessage"": ""Forbidden thing"",
                ""details"": [
