...
public class DeleteRecipientById_Integration_Test
{
    ...
    [Fact]
    public async Task DeleteRecipientByIdAsync_Test()
    {
        var apiKey = new InMobileApiKey("UnitTestKey123");
        var expectedRequest = new UnitTestRequestInfo(apiKey: apiKey, methodAndPath: "DELETE /v4/lists/some_list_id/recipients/recId1", jsonOrNull: null);
        var responseToSendback = new UnitTestResponseInfo(jsonOrNull: null, statusCodeString: "204 NoContent");
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            await client.Lists.DeleteRecipientByIdAsync(listId: new RecipientListId("some_list_id"), recipientId: new RecipientId("recId1"));
    ...
    [Fact]
    public async Task DeleteRecipientByIdAsync_ApiError_NotFound_Test()
    {
        var apiKey = new InMobileApiKey("UnitTestKey123");
        var expectedRequest = new UnitTestRequestInfo(apiKey: apiKey, methodAndPath: "DELETE /v4/lists/some_list_id/recipients/recId1", jsonOrNull: null);
        var responseToSendback = new UnitTestResponseInfo(jsonOrNull: @"{
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var ex = await Assert.ThrowsAsync<InMobileApiException>(() => client.Lists.DeleteRecipientByIdAsync(listId: new RecipientListId("some_list_id"), recipientId: new RecipientId("recId1")));
    ...
    [Fact]
    public async Task DeleteRecipientByIdAsync_ApiError_InternalServerError_Test()
    {
        var apiKey = new InMobileApiKey("UnitTestKey123");
        var expectedRequest = new UnitTestRequestInfo(apiKey: apiKey, methodAndPath: "DELETE /v4/lists/some_list_id/recipients/recId1", jsonOrNull: null);
        var responseToSendback = new UnitTestResponseInfo(jsonOrNull: @"{
