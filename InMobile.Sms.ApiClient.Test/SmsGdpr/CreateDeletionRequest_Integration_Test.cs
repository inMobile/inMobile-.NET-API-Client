...
public class CreateDeletionRequest_Integration_Test
{
    ...
    [Fact]
    public async Task CreateDeletionRequestAsync_Test()
    {
        var requestJson = @"{""NumberInfo"":{""CountryCode"":""45"",""PhoneNumber"":""12345678""}}";
        var responseJson = @"{                
                ""id"": ""dd5f73f7-0a83-4301-8d92-53d22b2c59f8""
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var entry = await client.SmsGdpr.CreateDeletionRequestAsync(new NumberInfo(countryCode: "45", phoneNumber: "12345678"));
    ...
    [Fact]
    public async Task CreateDeletionRequestAsync_ApiError_Test()
    {
        var requestJson = @"{""NumberInfo"":{""CountryCode"":""45"",""PhoneNumber"":""12345678""}}";
        var responseJson = @"{
                ""errorMessage"": ""Forbidden thing"",
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var ex = await Assert.ThrowsAsync<InMobileApiException>(() => client.SmsGdpr.CreateDeletionRequestAsync(new NumberInfo(countryCode: "45", phoneNumber: "12345678")));
