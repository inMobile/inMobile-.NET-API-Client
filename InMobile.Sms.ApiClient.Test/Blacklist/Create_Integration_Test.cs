...
public class Create_Integration_Test
{
    ...
    public async Task CreateAsync_Test()
    {
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var entry = await client.Blacklist.CreateAsync(new BlacklistEntryCreateInfo(new NumberInfo(countryCode: "45", phoneNumber: "12345678"), comment: "Sometextprovidedwhencreated"));
            Assert.Equal("45", entry.NumberInfo.CountryCode);
            Assert.Equal("12345678", entry.NumberInfo.PhoneNumber);
            Assert.Equal("Sometextprovidedwhencreated", entry.Comment);
            Assert.Equal("some_blacklist_id", entry.Id.Value);
...
""Go away""
    ...
    [Fact]
    public async Task CreateAsync_ApiError_Test()
    {
        var requestJson = @"{""NumberInfo"":{""CountryCode"":""45"",""PhoneNumber"":""12345678""},""Comment"":""Sometextprovidedwhencreated""}";
        var responseJson = @"{
""errorMessage"": ""Forbidden thing"",
...
""Go away""
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var ex = await Assert.ThrowsAsync<InMobileApiException>(() => client.Blacklist.CreateAsync(new BlacklistEntryCreateInfo(new NumberInfo(countryCode: "45", phoneNumber: "12345678"), comment: "Sometextprovidedwhencreated")));
            Assert.Equal(HttpStatusCode.InternalServerError, ex.ErrorHttpStatusCode);
        }
    }
}