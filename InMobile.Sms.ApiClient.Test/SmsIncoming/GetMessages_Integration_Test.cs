...
public class GetMessages_Integration_Test
{
    ...
    [Fact]
    public async Task GetMessagesAsync_Success_MsisdnSender_Test()
    {
        var responseJson = @"{
                    ""messages"": [
                    {
            ...
            using var server = StartOnAnyAvailablePort(new RequestResponsePair(request: expectedRequest, response: responseToSendBack));
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var response = await client.SmsIncoming.GetMessagesAsync(limit: 8);
    ...
    [Fact]
    public async Task GetMessagesAsync_Success_TextSender_Test()
    {
        var responseJson = @"{
                    ""messages"": [
                    {
...
""Go away""
        ...
        var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
        var ex = await Assert.ThrowsAsync<InMobileApiException>(() => client.SmsIncoming.GetMessagesAsync(limit: 1));
    ...
    [InlineData(249)]
    [InlineData(250)]
    public async Task GetMessagesAsync_Limit_ValidLimitsTest(int limit)
    {
        var emptyResponseJson = @"{
                    ""messages"": [
                    ]}";
        ...
        var reports = await client.SmsIncoming.GetMessagesAsync(limit);
        Assert.Empty(reports.Messages!);
    }
    ...
    [InlineData(251)]
    [InlineData(1000)]
    public async Task GetMessagesAsync_Limit_InvalidLimits_Test(int limit)
    {
        var apiKey = new InMobileApiKey("UnitTestKey123");
        var expectedRequest = new UnitTestRequestInfo(apiKey: apiKey, methodAndPath: $"GET /v4/sms/incoming/messages?limit={limit}", jsonOrNull: null);
        var responseToSendBack = new UnitTestResponseInfo(jsonOrNull: null);
        ...
        _ = await Assert.ThrowsAsync<ArgumentException>(() => client.SmsIncoming.GetMessagesAsync(limit: limit));
    }
}