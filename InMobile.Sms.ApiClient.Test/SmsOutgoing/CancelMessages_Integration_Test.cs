...
public class CancelMessages_Integration_Test
{
    ...
    [Fact]
    public async Task CancelMessagesAsync_Success_Test()
    {
        var expectedRequestJson = @"{""MessageIds"":[""id1"",""id2"",""id3""]}";
        var responseJson = @"{
                        ""results"": [
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var response = await client.SmsOutgoing.CancelMessagesAsync(messageIds: new List<OutgoingMessageId>() { new OutgoingMessageId("id1"), new OutgoingMessageId("id2"), new OutgoingMessageId("id3") });
            Assert.NotNull(response);
            Assert.Equal(3, response.Results.Count);
    ...
    [Fact]
    public async Task CancelMessagesAsync_ApiError_Test()
    {
        var expectedRequestJson = @"{""MessageIds"":[""id1"",""id2"",""id3""]}";
        var responseJson = @"{
                ""errorMessage"": ""Forbidden thing"",
        ...
        using (var server = UnitTestHttpServer.StartOnAnyAvailablePort(new RequestResponsePair(request: expectedRequest, response: responseToSendback)))
        {
            ...
            var ex = await Assert.ThrowsAsync<InMobileApiException>(() => client.SmsOutgoing.CancelMessagesAsync(messageIds: new List<OutgoingMessageId>() { new OutgoingMessageId("id1"), new OutgoingMessageId("id2"), new OutgoingMessageId("id3") }));
            Assert.Equal(HttpStatusCode.InternalServerError, ex.ErrorHttpStatusCode);
        }
    }
}