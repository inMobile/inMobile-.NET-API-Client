...
public class GetReports_Integration_Test
{
    ...
    [Fact]
    public async Task GetReportsAsync_Success_Test()
    {
        var responseJson = @"{
                    ""reports"": [
                    {
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var response = await client.SmsOutgoing.GetStatusReportsAsync(limit: 8);
            Assert.NotNull(response);
            Assert.Single(response.Reports);
            var report = response.Reports.Single();
            Assert.NotNull(report);
            ...
            Assert.Equal("4512345678", report.NumberDetails.Msisdn);
            Assert.Equal("DK", report.NumberDetails.CountryHint);
            Assert.True(report.NumberDetails.IsValidMsisdn);
            Assert.False(report.NumberDetails.IsAnonymized);
            ...
            Assert.Equal(MessageEncoding.Gsm7, report.ChargeInfo.Encoding);
            Assert.True(report.ChargeInfo.IsCharged);
            Assert.Null(report.ChargeInfo.Network);
            Assert.Equal(2, report.ChargeInfo.SmsCount);
        }
    }
    ...
    [Fact]
    public async Task GetReportsAsync_WithDifferentData_Test()
    {
        var responseJson = @"{
                    ""reports"": [
                    {
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var response = await client.SmsOutgoing.GetStatusReportsAsync(limit: 250);
            Assert.NotNull(response);
            Assert.Single(response.Reports);
            var report = response.Reports.Single();
            Assert.NotNull(report);
            ...
            Assert.Equal(new DateTime(2001, 02, 24, 14, 50, 23, DateTimeKind.Utc), report.DeliveryInfo.SendTime);
            Assert.Null(report.DeliveryInfo.DoneTime);
            Assert.Null(report.DeliveryInfo.ErrorCode);
            Assert.Null(report.DeliveryInfo.ErrorDescription);
            Assert.Equal(MessageEncoding.Unknown, report.ChargeInfo.Encoding);
            Assert.Equal("45-TDC", report.ChargeInfo.Network);
        }
    }
...
""Go away""
    ...
    [Fact]
    public async Task GetReportsAsync_ApiError_Test()
    {
        var responseJson = @"{
""errorMessage"": ""Forbidden thing"",
""details"": [
...
""Go away""
        ...
        {
            var client = new InMobileApiClient(apiKey, baseUrl: $"http://{server.EndPoint.Address}:{server.EndPoint.Port}");
            var ex = await Assert.ThrowsAsync<InMobileApiException>(() => client.SmsOutgoing.GetStatusReportsAsync(limit: 1));
    ...
    public void GetReports_Limit_Test(int limit, bool expectApiCalled)
    {
        ...
        using (var server = UnitTestHttpServer.StartOnAnyAvailablePort(new RequestResponsePair(request: expectedRequest, response: responseToSendback)))
        {
            ...
            {
                _ = Assert.Throws<ArgumentException>(() => client.SmsOutgoing.GetStatusReports(limit: limit));
            }
        }
    }
    ...
    [InlineData(251, false)]
    [InlineData(1000, false)]
    public async Task GetReportsAsync_Limit_Test(int limit, bool expectApiCalled)
    {
        var emptyResponseJson = @"{
                    ""reports"": [
                    ]}";
        ...
        using (var server = UnitTestHttpServer.StartOnAnyAvailablePort(new RequestResponsePair(request: expectedRequest, response: responseToSendback)))
        {
            ...
            if (expectApiCalled)
            {
                var reports = await client.SmsOutgoing.GetStatusReportsAsync(limit: limit);
                Assert.Empty(reports.Reports);
            }
            else
            {
                _ = await Assert.ThrowsAsync<ArgumentException>(() => client.SmsOutgoing.GetStatusReportsAsync(limit: limit));
            }
        }
    }
}