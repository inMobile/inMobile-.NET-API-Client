...
namespace InMobile.Sms.ApiClient
{
    ...
    {
        private const string _v4_lists = "/v4/lists";
        ...
        {
            return _requestHelper.ExecuteGetAndIteratePagedResult<RecipientList>(resource: $"{_v4_lists}?pageLimit=250");
        }

        public RecipientList GetListById(RecipientListId listId)
        {
            EnsureNonEmptyOrThrow(parameterName: nameof(listId), value: listId);
            return _requestHelper.Execute<RecipientList>(method: Method.GET, resource: $"{_v4_lists}/{listId}");
        }
        ...
        {
            EnsureNonEmptyOrThrow(parameterName: nameof(listId), value: listId);
            return _requestHelper.Execute<RecipientList>(method: Method.PUT, resource: $"{_v4_lists}/{listId}", payload: updateObject);
        }

        public void DeleteListById(RecipientListId listId)
        {
            EnsureNonEmptyOrThrow(parameterName: nameof(listId), value: listId);
            _requestHelper.ExecuteWithNoContent(method: Method.DELETE, resource: $"{_v4_lists}/{listId}");
        }
        ...
        public Recipient CreateRecipient(RecipientCreateInfo recipient)
        {
            ...
            return _requestHelper.Execute<Recipient>(method: Method.POST, resource: $"{_v4_lists}/{recipient.ListId}/recipients", payload: new { NumberInfo = recipient.NumberInfo, Fields = recipient.Fields });
        }

        public List<Recipient> GetAllRecipientsInList(RecipientListId listId)
        {
            EnsureNonEmptyOrThrow(parameterName: nameof(listId), value: listId);
            return _requestHelper.ExecuteGetAndIteratePagedResult<Recipient>(resource: $"{_v4_lists}/{listId}/recipients?pageLimit=250");
        }
        ...
        public Recipient GetRecipientById(RecipientListId listId, RecipientId recipientId)
        {
            ...
            return _requestHelper.Execute<Recipient>(method: Method.GET, resource: $"{_v4_lists}/{listId}/recipients/{recipientId}");
        }
        ...
        public Recipient GetRecipientByNumber(RecipientListId listId, NumberInfo numberInfo)
        {
            ...
            return _requestHelper.Execute<Recipient>(method: Method.GET, resource: $"{_v4_lists}/{listId}/recipients/bynumber?countryCode={numberInfo.CountryCode}&phoneNumber={numberInfo.PhoneNumber}");
        }
        ...
        private Recipient UpdateRecipientInternal(RecipientListId listId, RecipientId recipientId, object updateObject)
        {
            ...
            return _requestHelper.Execute<Recipient>(method: Method.PUT, resource: $"{_v4_lists}/{listId}/recipients/{recipientId}", payload: updateObject);
        }

        public void DeleteAllRecipientsInList(RecipientListId listId)
        {
            EnsureNonEmptyOrThrow(parameterName: nameof(listId), value: listId);
            _requestHelper.ExecuteWithNoContent(method: Method.DELETE, resource: $"{_v4_lists}/{listId}/recipients/all");
        }
        ...
        public void DeleteRecipientById(RecipientListId listId, RecipientId recipientId)
        {
            EnsureNonEmptyOrThrow(parameterName: nameof(listId), value: listId);
            EnsureNonEmptyOrThrow(parameterName: nameof(recipientId), value: recipientId);
            _requestHelper.ExecuteWithNoContent(method: Method.DELETE, resource: $"{_v4_lists}/{listId}/recipients/{recipientId}");
        }
        ...
        public void DeleteRecipientByNumber(RecipientListId listId, NumberInfo numberInfo)
        {
            EnsureNonEmptyOrThrow(parameterName: nameof(listId), value: listId);
            EnsureNonEmptyOrThrow(parameterName: nameof(numberInfo), value: numberInfo);
            _requestHelper.ExecuteWithNoContent(method: Method.DELETE, resource: $"{_v4_lists}/{listId}/recipients/bynumber?countryCode={numberInfo.CountryCode}&phoneNumber={numberInfo.PhoneNumber}");
        }
